...
known_landmark_colors = [CRED, CGREEN, CBLUE, CBLACK]

# The robot knows the position of 2 landmarks. Their coordinates are in the unit centimeters [cm].
known_landmarkIDs = [1, 2, 3, 4]
known_landmarks = {
...
def draw_world(est_pose, particles, world):
    ...
    # Draw landmarks
    for i in range(len(known_landmarkIDs)):
        ID = known_landmarkIDs[i]
        lm = (int(known_landmarks[ID][0] + offsetX),
              int(ymax - (known_landmarks[ID][1] + offsetY)))
        cv2.circle(world, lm, 5, known_landmark_colors[i], 2)
...
def compute_center():
    ...
    '''
    center_x = (known_landmarks[1][0] + known_landmarks[2][0]) / 2
    center_y = (known_landmarks[1][1] + known_landmarks[2][1]) / 2
...
def compute_weight(objectIDs, i, p, dists, angles):
    ...
    x = known_landmarks[objectIDs[i]][0] - p.getX()  # XXX
    y = known_landmarks[objectIDs[i]][1] - p.getY()  # XXX

    dist = np.sqrt(pow(x, 2) + pow(y, 2))
    # XXX
    dist_weight = np.exp(-(pow(dists[i] - dist,
                         2) / (2 * pow(spread_dist, 2))))
    ...
    orientation_weight = np.exp(-(pow(orientation, 2) /
                                (2 * pow(spread_angle, 2))))
...
try:
    ...
    while 1:
        ...
        if not isinstance(objectIDs, type(None)):
            ...
            objectIDs, dists, angles = delete_duplicates(
