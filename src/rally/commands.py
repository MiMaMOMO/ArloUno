...
#     sign = np.sign(angle)
#     # Get absolute degrees from radians
...
#     # Scale the distance down to meters
#     scaled_dist = dist / 100
#     # How close we want Arlo to get to the landmark
...
def rotate(arlo, angle) -> None:
    ...
    sign = np.sign(angle)                           # Get the sign of the angle
    # Make radians into degrees. Get absolute value
    scaled_angle = np.abs(np.degrees(angle))
    # Seconds it takes Arlo to rotate angle amount
    rot_time = scaled_angle * (TIME_ORIENTATION / 90)
    # Timer to measure a countdown for the rotation
    t = Timer()
...
def drive(arlo, dist, landmark_range=0.0) -> None:
    ...
    scaled_dist = dist / 100                    # Scale the distance down to meters
    # How long in seconds it takes Arlo to drive dist
    drive_time = scaled_dist * TIME_METER
    # Timer used to measure a countdown for Arlo
    t = Timer()
